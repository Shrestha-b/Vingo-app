...
const handleGoogleSignUp = async () => {
    ...
    try {
        const {data} = await axios.post(`${serverUrl}/api/auth/google-auth`, {
          ...
          email:result.user.email,mobile,role},{withCredentials:true})
          dispatch(setUserData(result.data))
    } catch (error) {
          console.log(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>",error)
    }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <button className={`w-full mt-4 gap-2 font-semibold flex justify-center text-center px-4 py-2 border rounded-lg transition duration-200 bg-[#ff4d2d] text-white hover:bg-[#e64323]`} onClick={handleSignUp} disabled={loading} >
             {loading  ? <ClipLoader 
                    color={color}
                    size={20}
                    /> : "Sign Up"}
